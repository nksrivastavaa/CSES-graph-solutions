...
vector<vpl> adj;
vi countOfOccurences;

void kth_Dijkstra(int &n, int &k){
    priority_queue<pl, vpl, greater<pl>> pq;
    pq.push(mp(0, 1)); //no need to make distance array. push dist = 0 of node 1 in the beginning.
    while(!pq.empty() && countOfOccurences[n] < k){
        ll d = pq.top().fs;
        ll curr = pq.top().sn;
        pq.pop();
        if(curr == n) cout<<d<<" ";
        if(countOfOccurences[curr] < k){
            /*if curr node exists in less than k paths,
            you can use it to relax other nodes.*/
            for(auto i: adj[curr]){
                pq.push(mp(d + i.sn, i.fs));
            }            
        }
        countOfOccurences[curr]++;
    }
    cout<<nl;
}
void solve() {
    /*pick trash values of dijkstra till either pq!= empty or 
    count of last vertex (in all the paths) <= k cause we need to print distances of 
    k shortest paths only. also ensure other vertices in the path are visited
    <=k times*/
    int n, m, k;
    cin>>n>>m>>k;
    adj.assign(n+1, vpl());
    countOfOccurences.assign(n+1, 0);
    rep(i, m){
        int a, b, c;
        cin>>a>>b>>c;
        adj[a].pb(mp(b, c));
