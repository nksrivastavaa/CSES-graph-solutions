...
void dijkstra(){
    ...
    while(!pq.empty()){
        ...
        for(auto i: adj[node]){
            if(dist[i.fs] > dist[node] + i.sn){
                dist[i.fs] = dist[node] + i.sn;
                noOfWays[i.fs] = noOfWays[node]%mod;
                maxf[i.fs] = maxf[node] + 1;
                minf[i.fs] = minf[node] + 1;
                pq.push({dist[i.fs], i.fs});
            }
            else if(dist[i.fs] == dist[node] + i.sn){
                noOfWays[i.fs] += noOfWays[node];
                noOfWays[i.fs] %= mod;
                maxf[i.fs] = max(maxf[i.fs], maxf[node] + 1);
                minf[i.fs] = min(minf[i.fs], minf[node] + 1);
            }
        }
    }
}
...
void solve() {
    ...
    cout<<dist[n]<<" "<<noOfWays[n]<<" "<<minf[n]<<" "<<maxf[n]<<nl;
}
