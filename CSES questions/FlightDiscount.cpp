...
vector<vpl> adj;
vector<vpl> tadj; //transpose graph
vl dist; //dist from 1 to every city
vl distn; //dist from n to every city
vector<pair<pl, ll>> edgeList;
...
void tdijkstra(ll n) {
    ...
    while(!pq.empty()){
        ...
        ll d = pq.top().fs;
        pq.pop();
        if(d > distn[curr]) continue; //optimization: cause this d is useless & we dont need to relax other distances based on this.
        for(auto i: tadj[curr]){
            if(distn[i.fs] > distn[curr] + i.sn){
                distn[i.fs] = distn[curr] + i.sn;
                pq.push(mp(distn[i.fs], i.fs));
            }
        }
    }
}
...
void solve() {
    ...
    for(auto i: edgeList){
        /*FOR ALL EDGES U-V IN PATH FROM 1 TO N,
        MAKE DISTANCE(1, N) = 
        min(distance(1, n), distance(1, u) + weight(u-v)/2 + distance(v, n))*/
        dist[n] = min(dist[n], dist[i.fs.fs] + i.sn/2 + distn[i.fs.sn]);
    }
    cout<<dist[n]<<nl;
}
