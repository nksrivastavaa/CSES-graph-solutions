...
void bfs(int i, int j){
    ...
    while(!q.empty()){
        pi curr = q.front();
        q.pop(); 
        rep(k, 4) //k = 0-down, 1-right, 2-up, 3-left
        {
            if( (curr.fs+dy[k] < inp.size() && curr.fs+dy[k]>=0) && (curr.sn+dx[k] < inp[0].size() && curr.sn+dx[k]>=0) && !vis[curr.fs+dy[k]][curr.sn+dx[k]] && (inp[curr.fs+dy[k]][curr.sn+dx[k]] == '.' || inp[curr.fs+dy[k]][curr.sn+dx[k]] == 'B') ){
                if(inp[curr.fs+dy[k]][curr.sn+dx[k]] == 'B'){
                    canReachB = true;
                }
                q.push(mp(curr.fs+dy[k], curr.sn+dx[k]));
                prevStep[curr.fs+dy[k]][curr.sn+dx[k]] = direction[k]; //par to curr -> direction
                vis[curr.fs+dy[k]][curr.sn+dx[k]] = true;
            }
        }   
    }
}
...
void solve() {
    ...
    string ans = "";
    while(bx!=ax || by!=ay){
        ans.pb(prevStep[by][bx]);
        if(prevStep[by][bx] == 'U') by++;
        else if(prevStep[by][bx] == 'D') by--;
        else if(prevStep[by][bx] == 'R') bx--;
        else if(prevStep[by][bx] == 'L') bx++;
    }
    reverse(all(ans));
    cout<<"YES"<<nl;
    cout<<ans.size()<<nl;
