...
void dfs(int i){
    ...
    for(auto j: adj[i]){
        if(!vis[j]){
            ...
            if(prevStep[i] != j && prevStep[j] != i){
                cycleDetected = true;
                cycleDetectedAt.fs = i;
                cycleDetectedAt.sn = j;
            }    
        }
    }
}
...
void solve() {
    ...
    vi ans;
    while(cycleDetectedAt.sn != cycleDetectedAt.fs){
        ans.pb(cycleDetectedAt.sn);
        cycleDetectedAt.sn = prevStep[cycleDetectedAt.sn];
    }
    ans.pb(cycleDetectedAt.fs);
    ans.pb(ans[0]);
    dbg(ans)
    cout<<ans.size()<<nl;
    rep(i, ans.size()){
