...
vvi adj;
vb vis;
vb visInSameRecursionStack;
stack<int> topologicalOrder;

void topologicalSort(int n){
    vis[n] = 1;
    visInSameRecursionStack[n] = 1;
    for(auto i: adj[n]){
        if(!vis[i]){
            topologicalSort(i);
        }
        else if(vis[i] && visInSameRecursionStack[i]){//CYCLE DETECTED
            cout<<"IMPOSSIBLE"<<nl;
            exit(0);
        }
    }
    topologicalOrder.push(n);
    visInSameRecursionStack[n] = 0;
}
...
void solve() {
    ...
    visInSameRecursionStack.assign(n+1, false);
    rep(i, m){
        int a, b;
        cin>>a>>b;
        adj[a].pb(b);
