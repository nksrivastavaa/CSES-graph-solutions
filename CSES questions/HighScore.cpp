...
vvi adj;
vvi tAdj; //transpose graph
vl dis;
vb vis;
vb tVis;
vector<pair<pi, ll>> edgeList; //push -ve weights in edgelist to detect positive cycles. cause bellmann can detect -ve cycles

void BellmannFord(int &n){
    rep(i, n){
        for(auto e: edgeList){
            if(dis[e.fs.sn] > dis[e.fs.fs] + e.sn){
                dis[e.fs.sn] = dis[e.fs.fs] + e.sn;
                if(i == n-1 && vis[e.fs.sn] && tVis[e.fs.sn]){
                    /*Bellman gets shortest dist
                    in n-1 iterations where n is no.
                    of nodes. Run it for 1 more iteration
                    and if distance is still getting reduced,
                    it means there is a negative cycle causing this*/
                    /*to check if negative cycles
                    are in the route from
                    starting node to end node check vis and tVis*/
                    cout<<-1<<nl;
                    return;
                }
            }
...
void solve() {
    /*Basically this question uses
    MODIFIED version of Bellmann Ford 
    TO GET LONGEST PATH IN DIRECTED CYCLIC GRAPH.*/
    int n, m;
    cin>>n>>m;
    adj.assign(n+1, vi());
    tAdj.assign(n+1, vi());
    ...
    dfs(1);
    revDfs(n);
    /*use dfs and revDfs to check if positive cycles
    (in this question) are in the route from
    starting node to end node*/
    BellmannFord(n);
}
