...
vector<pair<pi, ll>> edgelist;
vl dist;
vi parent;
int neg = -1; //store the vertex which is involved in relaxing other vertices.
 
void BellmannFord(int &n){
    rep(i, n){
        for(auto e: edgelist){
            if(dist[e.fs.sn] > dist[e.fs.fs] + e.sn){
                dist[e.fs.sn] = dist[e.fs.fs] + e.sn;
                parent[e.fs.sn] = e.fs.fs;
                if(i == n-1){//NEGATIVE CYCLE DETECTED.
                    neg = e.fs.fs;
                }
            }
        }
...
void retracePath(int a, int &n){
    /*IMPORTANT CONCEPT TO LEARN: 
    HOW TO PRINT CYCLES (OR RATHER NEGATIVE CYCLES AS IN THIS QUESTION.)*/
    
    /*it is not necessary that "a" is a vertex inside the negative cycle. 
    it might be a vertex that is getting relaxed from 
    a vertex inside the negative cycle*/
    
    rep(i, n){
        /*since maximum negative cycle will be of size <= n, therefore trackback
        from "a" n times to be assured you're at a vertex inside the cycle*/
        a = parent[a];
    }
    vi ans;
    int node = a;
